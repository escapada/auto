    ...
    module Admin
      class ProductsController < ::Refinery::AdminController
        ...
        #logger.debug("///////////////////////////FIND_ALL_PRODUCTS and FILTERED/////////////////////////////////////////////////////////////#{@mmm}")
          @products = Product.includes(:carmodels, :carmodelsubtype).order('position DESC') if (!params[:model]&&!params[:type])
          @products = Product.includes(:carmodels, :carmodelsubtype).where('carmodels.id' => params[:model]).order('id DESC') if (params[:model]&&!params[:type])
          @products = Product.includes(:carmodels, :carmodelsubtype).where('carmodels.id' => params[:model]).order('id DESC') if (!params[:model]&&params[:type])
          @products = Product.includes(:carmodels, :carmodelsubtype).where('carmodels.id' => params[:model], carmodelsubtype_id: params[:type] ).order('id DESC') if (params[:model]&&params[:type])  
          
        end

        ################set_filter must set the session cookie (can be nil (all products) | filtered by model | filtered by type | filtered by model+type)
        def set_filter
